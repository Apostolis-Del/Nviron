...
function OrgLikeButton({ user, orgpost: { id, likeCount, likes } }) {
  ...
  //
  const [likePost] = useMutation(LIKE_ORGPOST_MUTATION, {
    variables: { postId: id }
  });
...
const LIKE_ORGPOST_MUTATION = gql`
  mutation likeOrgPost($postId: ID!) {
    likeOrgPost(postId: $postId) {
      id
      likes {
        id
        username
